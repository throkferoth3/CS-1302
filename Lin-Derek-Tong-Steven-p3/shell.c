...
int main (int argc, char* argv[]) {
	...
	if (argc > 1) {
		if ((pid = fork()) < 0) {
            ...
            for (i = 2; i < argc; i++) { //Going through all arguments.
                if (strcmp(argv[i], "<") == 0) { // Redirect by input.
                    int j = i;
                ...
                } else if ((strcmp(argv[i], ">") == 0)) { // Redirect by output.
                    int j = i;
                    for (j = i; j < argc; j++) argv[j] = argv[j + 1];
                ...
                } else if ((strcmp(argv[i], ">>") == 0)) { // Redirect by append output.
                    int j = i;
                    for (j = i; j < argc; j++) argv[j] = argv[j + 1];
    ...
    } else { // If there is no command specified.
